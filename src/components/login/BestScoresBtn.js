...
class BestScoresBtn extends Component {
    ...
    componentDidMount = () => {
        ...
        if ( this._isMounted ) {
            firebase.database().ref('/game/bestScores').on('value', snap => {
                ...
                console.log(snap.val(), 'Memory leak')
                if ( this._isMounted && snap.val() ) { this.setState({ bestScores: snap.val() }) }
            })
        }
    }
    ...
    showOrHideBestScores = () => this.setState({ showBestScores: !this.state.showBestScores })
