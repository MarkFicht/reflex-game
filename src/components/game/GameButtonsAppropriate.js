...
import { randomChar, randomCharLength } from '../../components/other/randomChar'
import good from '../../sound/good.wav'
import wrong from '../../sound/wrong.mp3'


class GameButtonsAppropriate extends Component {
    _isMounted = false
    good = new Audio(good)
    bad = new Audio(wrong)
    ...
    componentDidMount = () => {
        ...
        if (this._isMounted) window.addEventListener('keydown', this.keyEvent)
    }
    ...
    addOrSubtractPoint = (clickedChar) => {
        ...
        if (clickedChar === correctChar) {
            ...
            let newChar = randomChar[Math.floor(Math.random() * randomCharLength + 1) - 1]
        ...
        return (
            <div className="btns">
                {randomChar.map(char => {
                    ...
                    return <button 
                                onClick={e => this.mouseEvent(char)} 
                                disabled={!startTime && true} 
                                style={style} 
                                className='btn-game' 
                                key={char}
                            >{ char }</button>
...
export default GameButtonsAppropriate